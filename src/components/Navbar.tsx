    ...
    <header className="px-4 lg:px-6 h-14 flex py-2 fixed-top z-50">
      <Link href="/">
        <svg
          ...
          className={`width-14 h-14 sm:w-16 sm:h-16 animated-svg ${!isErrorPage(path) ? "animated-svg-draw" : "animate-bounce"}`}
        >
      ...
      <nav
        className={`ml-auto flex gap-4 sm:gap-6 p-4 text-lg font-medium ${!isErrorPage(path) ? "animation-text-fade-in-x" : ""}`}
      >
